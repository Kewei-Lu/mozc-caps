...
//  const float kAngleRange = 80;
  const float kAngleRange = 12;
  int num_keys = sizeof(characters) / sizeof(characters[0]);
  int position = relative_angle / kAngleRange;
//  float position = (relative_angle + kAngleRange) / kAngleRange / 2 * num_keys;
  int key_index = static_cast<int>(position);
  int raw_index = key_index;
  //  if key_index < 0, let's assume we are selecting the last character
  //  if (key_index < 0) key_index = 0;
  if (key_index < 0) key_index = num_keys - 1;
...
//  }
  ...
  if (imu.Ready()) {
    M5.Lcd.printf("current key: %d ,raw_index: %d,relative_angle: %f ",current_key,raw_index,relative_angle);
  } else {
  ...
  char *displayChara = characters[current_key];
  if (*displayChara == ' '){
    displayChara = "bl";
  }
  M5.Lcd.printf(" %3s ", displayChara);
